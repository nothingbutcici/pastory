    ...
    static func handleCommandLine() -> Bool {
        ...
        Task { @MainActor in
            ...
            case "settings":
                ...
                host.frame = CGRect(x: 0, y: 0, width: CGFloat(Double(ProcessInfo.processInfo.environment["PASTORY_WIDTH"] ?? "") ?? 1600), height: CGFloat(Double(ProcessInfo.processInfo.environment["PASTORY_HEIGHT"] ?? "") ?? 450))      // taller = see the lower sections
            ...
            case "note":
                ...
                ok = snapshot(host, to: rest.first ?? "pastory-note.png")
            case "welcome":
                await seedStore()
                let model = ShelfPanelController.shared.model
                model.reset()
                ...
                let host = NSHostingView(rootView: ShelfView(model: model))
                host.frame = CGRect(x: 0, y: 0, width: 1600, height: CGFloat(Double(ProcessInfo.processInfo.environment["PASTORY_HEIGHT"] ?? "") ?? 450))
                let w = NSWindow(contentRect: host.frame, styleMask: [.borderless], backing: .buffered, defer: false)
                w.contentView = host
                w.isReleasedWhenClosed = false
