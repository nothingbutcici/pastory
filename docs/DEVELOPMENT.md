...
- **桌面便签**（`Shelf/DesktopNotes.swift`、`DesktopNoteView.swift`）：把面板里的卡向上拖出面板边界即成为桌面便签（松手在面板上=取消），右键也有「贴到桌面」；便签是同一张纸卡、全文展示（文本最高屏高 60%，超出卡内滚动），底部只有 编辑 / 关闭；单击复制、双击贴进前台应用；非激活面板，不抢焦点。贴上即自动 Pin；面板里的卡保留并带 note 标记；卡被删便签同删。层级逐张决定（便签底部的图层键 / 右键）：浮于窗口上（默认，拖出时一定看得见）或仅桌面显示（桌面图标之上、所有应用窗口之下）；所有 Space 可见。发现路径：欢迎卡「试一试」多一条、卡片右键「贴到桌面」、以及首次启动本版本时种进历史的 Pin 卡「Pastory 怎么用」（`Shelf/HowToCard.swift`，普通卡片，删了不再出现）。位置存在偏好 `desktopNotes`，启动恢复并夹回可见屏幕。`--selftest note` 渲染。
- **面板键盘**：双击 = 复制、收起并粘贴到刚才的应用；⏎ = 复制并收起，只有设置选了「双击 + 回车」且这张卡是用方向键亲手选中的才同样粘贴（搜索、筛选、删除后自动落到的高亮都不算；点击卡片后键盘会交还给原应用，所以鼠标选中后的回车到不了面板）；← → ↑ ↓ 选；空格 Quick Look；⌘P Pin；⌘S 保存到本地；⌫ 删除（Pin 项确认）；⌘F 搜索；直接打字即搜索；⎋ 依次关闭重命名框 / 返回 / 清空搜索 / 收起。设置页打开时只认 ⎋ 和 ⌘F。
