...
- **面板键盘**：双击 = 复制、收起并粘贴到刚才的应用；⏎ = 复制并收起，只有设置选了「双击 + 回车」且这张卡是用方向键或鼠标亲手选中的才同样粘贴（搜索自动高亮的第一条不算，搜索词一变即清零）；← → ↑ ↓ 选；空格 Quick Look；⌘P Pin；⌘S 保存到本地；⌫ 删除（Pin 项确认）；⌘F 搜索；直接打字即搜索；⎋ 依次关闭重命名框 / 返回 / 清空搜索 / 收起。设置页打开时只认 ⎋ 和 ⌘F。
...
- **文字标注**：点击输入，⏎ 换行，⌘⏎ 或点击框外确认（⎋ 放弃）；点击框外只确认、不新建，再点一次才开始下一个；确认后 ⏎ = 完成整张截图。新文字框宽度跟随文字，碰到图片右缘才换行；拖过边角后宽度固定。文字框的四角和四边都可拖动，宽度变化时保留字号并自动换行，高度至少容纳完整内容。编辑与导出共用 TextKit 排版，重新编辑保留框尺寸。
