...
- 视觉：深色浮岛 + 荧光绿强调（`App/Theme.swift` 统一取色），logo 在 `Resources/Logo.png`。
...
- **⇧⌘V 剪贴板**：底部滑出（屏高 55%）浅色货架：左侧浅绿侧栏（logo、今日暂存、清理规则、Pin 说明），
  顶部胶囊筛选 全部 / Pin / 图片 / 录屏 / 文本 + 搜索 + 设置 + ✕，白色卡片（app 图标、时间、内容、
  「已复制」标记 = 此刻剪贴板里的那条、说明行、Pin / 保存 / 删除三个大键），底部滚动条带左右箭头。左 = 最新。
  单击卡片 = 复制回剪贴板并收起。
  ← → 选，⏎ 复制，空格 Quick Look 预览，P 固定，S 保存到本地（弹对话框选位置），⌫ 删除，⌘F 搜索，⎋ 关闭。
  过滤：全部 / 固定 / 图片 / 录屏 / 文本。
