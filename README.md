...
- 工具条两层，结构对标飞书：主条 = 录屏 │ 矩形 R / 椭圆 O / 箭头 A / 直线 L / 画笔 P / 文字 T / 马赛克 M │ 识别文字 │
  保存到文件 · 红 ✕ 取消 · 绿 ✓ 完成。子条只在点了某个工具、或选中了某个元素时从主条下方弹出（带指向小三角），
  内容是三档粗细（文字工具显示 小 / 中 / 大）、7 个方形色块（选中打勾）、虚线开关；改动作用于选中元素，
  没选中时作为新元素的默认值。再点一次当前工具 = 收起。
...
- **录屏**：截图工具条最左边的红点。点了之后遮罩撤掉、选区外围留一圈紫框、下方一个小条（计时 / 停止 / 丢弃），
  ...
  预览窗保存后显示实际大小。文件进货架并以文件形式进剪贴板，可直接 ⌘V 发出去。
