...
- 「保存到本地」= 把条目导出到 `~/Downloads/`（可在设置里改），存储里的原条目不动。
...
- 工具条（白色浮岛，Excalidraw 风）：选择 V / 矩形 R / 椭圆 O / 箭头 A / 直线 L / 画笔 P / 文字 T / 马赛克 M，
  7 色，S M L 一个大小控制（同时管线宽和字号），虚线开关。画完一个形状自动回到「选择」并选中它，
  可以直接拖动；再点别的形状也能选中拖动；⌫ 删除选中；双击文字重新编辑；画笔画完保持画笔。
...
- **⇧⌘V 剪贴板**：底部滑出半屏货架，左 = 最新。单击卡片 = 复制回剪贴板并收起。
  ← → 选，⏎ 复制，P 固定，S 保存到本地，⌫ 删除，⌘F 搜索，⎋ 关闭。右键有完整菜单。
