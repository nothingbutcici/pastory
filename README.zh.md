...
<p align="center"><b>Pastory = Paste + History</b> · <a href="README.md">English</a></p>
...
2. 打开它。菜单栏右上角出现手写的 **P**，第一次会自动弹出剪贴板面板。（包已经过 Apple 公证，没有安全提示。）
3. 第一次截图时，按提示打开「屏幕录制」权限，然后重新打开 Pastory。

默认快捷键：**⌥⌘S** 截图 / 录屏，**⇧⌘V** 剪贴板。都可以在设置里改。

可选：打开「辅助功能」权限后，双击卡片才能直接贴进输入框；不开的话双击就是复制并收起。

有新版本时 Pastory 会提示你，点一下就地更新。
